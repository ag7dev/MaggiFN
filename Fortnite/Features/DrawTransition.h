...
#define GRD_TO_BOG(deg) ((deg) * M_PI / 180.0)  
#define BOG_TO_GRD(rad) ((rad) * 180.0 / M_PI)  
...
void DrawTransition( UGameViewportClient* ViewPortClient, UCanvas* Canvas )
{
			...
			if (!AcknowledgedPawn->IsPassengerInVehicle())
			{
				...
				if (Settings::Exploits::InfiniteFuel && AcknowledgedPawn)
				{
					...
					if (CurrentVehicle)
					{
						...
						{
							CachedFuelComponent->SetServerFuel(3.402823466e+38F); 
							CachedFuelComponent->SetInfiniteFuel(3.402823466e+38F); 
						}
					}
				}
			}
	...
	for ( int32_t i = 0; i < PlayerList.NumElements; i++ )
	{
		...
		if ( PlayerCheck && Player != AcknowledgedPawn )
		{
			...
			else if (Settings::Cache::bChamsWasActive) 
			{
			...
			else if (Settings::Cache::bChamsWasActive && AcknowledgedPawn) 
			{
				Settings::Cache::bChamsWasActive = false;
					...
					if (MaterialInstances.Valid(0))
					{
						for (int mi = 0; mi < MaterialInstances.Size(); mi++)
						{
							...
							if (MaterialInstances.NumElements - 2 >= mi)
							{
								...
								*(BYTE*)(LMaterial + Offsets::ExploitOffsets::Wireframe) |= 1 << 6; 
								*(BYTE*)(LMaterial + Offsets::ExploitOffsets::bDisableDepthTest) |= 1 << 0; 
							}
				...
				if ( Settings::Player::Enable )
				{
					...
					if (Settings::Developer::Debugging::HealthTestOthers && Player && Player->Mesh() && Player->PlayerState() && PlayerController && AcknowledgedPawn)
					{
						...
						if (PlayerController->w2s(Head, &Head2D) && PlayerController->w2s(RightFoot, &RightFoot2D))
						{
							...
							TargetRenderPosBegin.X = RightFoot2D; 
	...
	if ( Settings::Aimbot::Aimbot )
	{
		if ( AcknowledgedPawn )
		{
			...
			if (Settings::Developer::FreecamTest && PlayerController && AcknowledgedPawn) 
			{
			...
			if ( CurrentWeapon )
			{
				...
				if ( IsHoldingWeapon )
				{
					...
					if (Settings::Exploits::BulletTPV2_1 && !Settings::Exploits::BulletTPV2 && !Settings::Exploits::AirStuck) 
					{
						APlayerPawn_Athena_C* Target = nullptr;
						...
						if (Target)
						{
							...
							if (Mesh)
							{
								...
								if (PlayerController->IsInputKeyDown(Keys::Capslock) && Target && Target->Mesh() && AcknowledgedPawn)
								{
									...
									TargetTeleportLoc = PlayerController->GetLocationUnderReticle(); 

									
									TargetLookAt = StaticClasses::KismetMathLibrary->FindLookAtRotation(TargetTeleportLoc, Variables::CameraLocation); 

									FVector2D TargetTeleportLocOnScreenStart; 
									FVector2D TargetTeleportLocOnScreen; 
									...
									Target->K2_SetActorLocation(TargetTeleportLoc, false, 0, true); 
								}
							}
						}
					}
										...
										if (Settings::Exploits::TargetStrafe && Target && AcknowledgedPawn && Target->Mesh() && World && PlayerController && PlayerController->PlayerCameraManager())
										{
											...
											if (15.0 >= Distance)
											{
												...
												if (StaticClasses::KismetSystemLibrary->GetGameTimeInSeconds(World) - LastTargetStrafeActionTime >= 0.10)
												{
													FVector CurrentLocation = AcknowledgedPawn->K2_GetActorLocation(); 
													FVector TargetLocation = Target->K2_GetActorLocation(); 
													FRotator TargetRotation = Target->K2_GetActorRotation(); 
													FRotator CameraCurrentRotation = PlayerController->PlayerCameraManager()->GetCameraRotation(); 
												...
												if (Variables::CameraLocation && Variables::CameraRotation && AimbotPosition)
												{
													...
													*(float*)((uintptr_t)GameState + 0x1798) = Settings::Aimbot::Smoothing * 2.f;*/
													...
													if (TargetRotation)
													{
														...
														if (PlayerController)
														{
															if (Settings::Exploits::bBackTrack) 
															{
																...
																if (bWantsBacktrack)
																{
																	Target->SetCustomTimeDilation(0.001f); 
	...
	if (Settings::Player::CrosshairMode)
	{
		...
		else if (Settings::Player::CrosshairMode == 5 && World && Canvas)
		{
			...
			{
				std::vector<int> p;
				p.push_back(static_cast<int>(a * sin(GRD_TO_BOG(RotationAngle + (i * 90)))));                                    
				p.push_back(static_cast<int>(a * cos(GRD_TO_BOG(RotationAngle + (i * 90)))));                                    
				p.push_back(static_cast<int>((a / cos(gamma)) * sin(GRD_TO_BOG(RotationAngle + (i * 90) + BOG_TO_GRD(gamma))))); 
				p.push_back(static_cast<int>((a / cos(gamma)) * cos(GRD_TO_BOG(RotationAngle + (i * 90) + BOG_TO_GRD(gamma))))); 
				...
				Wrapper::Line(p0, p1, lineColor, 2.0f); 
		...
		if (Custom::DebugUIBegin()) 
		{
			if (Custom::DebugState(Settings::Developer::Debugging::ActorDebugging) && AcknowledgedPawn)
			{
				...
				if (auto CurrentWeapon = AcknowledgedPawn->CurrentWeapon())
				{
					...
					Custom::WriteDebugUIStringEx(PlatformUserStr, MaggiDebugMessageType::Error); 
					...
					Custom::WriteDebugUIStringEx(PlatformUserDirStr, MaggiDebugMessageType::Error); 
					...
					Custom::WriteDebugUIString_Bool(SlidingEnabledStr, MaggiDebugMessageType::Misc); 
					...
					Custom::WriteDebugUIString_Bool(CanCrouchStr, MaggiDebugMessageType::Misc); 

					FString CanJumpStr = StaticClasses::KismetStringLibrary->BuildString_Int(FString(L"CanJump: "), L"", AcknowledgedPawn->CanJump(), L"");
					Custom::WriteDebugUIString_Bool(CanJumpStr, MaggiDebugMessageType::Misc); 
					...
					if (Settings::Developer::Debugging::UnsafeActorDebugging)
					{
						...
						Custom::WriteDebugUIString_Bool(CanJumpStr, MaggiDebugMessageType::Error); 
						...
						Custom::WriteDebugUIString_Bool(SlidingEnabledStr, MaggiDebugMessageType::Error); 
			...
			if(Custom::DebugState(Settings::Developer::Debugging::SpreadDebugging) && AcknowledgedPawn)
			{
				if (auto CurrentWeapon = AcknowledgedPawn->CurrentWeapon())
				{
					Custom::WriteDebugUIStringEx(FString(L"\r\nCurrentWeapon Debug"), MaggiDebugMessageType::Misc); 
	...
	if (Settings::Exploits::Spinbot && AcknowledgedPawn)
	{
		...
		uintptr_t RelativeRotation = reinterpret_cast<uintptr_t>(AcknowledgedPawn->Mesh()) + Offsets::ExploitOffsets::RelativeRotation; 
	...
	if (Settings::Exploits::FullAutoWeapons && AcknowledgedPawn)
	{
		if (auto CurrentWeapon = AcknowledgedPawn->CurrentWeapon())
		{
			if (uintptr_t WeaponData = *(uintptr_t*)(CurrentWeapon + Offsets::ExploitOffsets::WeaponData)) 
			{
				...
				if (bContinue)
				{
					...
					*(uint8_t*)(WeaponData + Offsets::ExploitOffsets::TriggerType) = 1;  
					*(uint8_t*)(WeaponData + Offsets::ExploitOffsets::TriggerType + 0x1) = 1;  
				}
			}
		}
	}
		...
		if ((Settings::Exploits::RapidFireWithKeybind && PlayerController->IsInputKeyDown(Keys::Capslock)) || !Settings::Exploits::RapidFireWithKeybind)
		{
			if (auto Wepon = AcknowledgedPawn->CurrentWeapon())
			{
				...
				*reinterpret_cast<float*>(LastFireTimeVerified) = 0;
				...
				{
					*reinterpret_cast<float*>(Wepon + Offsets::ExploitOffsets::CustomTimeDilation) = 15; 
				}
				else
				{
	...
	if ((Settings::Exploits::SlideBHOP && PlayerController && AcknowledgedPawn) && !AcknowledgedPawn->IsSkyDiving()) 
	{
		...
		if (bContinue)
		{
			if ((!Settings::Developer::bDisableSlideEnabledCheck || AcknowledgedPawn->IsSlidingEnabled())) 
			{
				...
				if ((AcknowledgedPawn->CanJump() || AcknowledgedPawn->CanCrouch())) 
				{
					if (StaticClasses::KismetSystemLibrary->GetGameTimeInSeconds(World) - LastBHopIntervalTick >= Settings::Exploits::SlideBHOPInterval)
					{
						...
						{
							if (AcknowledgedPawn->CanCrouch()) 
							{
								AcknowledgedPawn->SetCrouch(true, Settings::Developer::bUseSecondArgTestForBHOP); 
	...
	if (Settings::Exploits::NoWeaponOverheat && AcknowledgedPawn && AcknowledgedPawn->Mesh())
	{
		if (auto Wepon = AcknowledgedPawn->CurrentWeapon())
		{
			if (Wepon->GetIsChargingWeapon())
			{
				if (*reinterpret_cast<float*>(Wepon + Offsets::ExploitOffsets::CustomTimeDilation) != 999) 
				{
					*reinterpret_cast<float*>(Wepon + Offsets::ExploitOffsets::CustomTimeDilation) = 999;  
				}
			}
			else
			{
			...
			{
				*reinterpret_cast<bool*>(Wepon + Offsets::ExploitOffsets::bCooldownWhileOverheated) = false;  
	...
	if (Variables::DrawMenu)
	{
		...
		if (Wrapper::IsSafeToRender())
		{
			...
			{
				auto EncryptedDevName = Encrypt(L"maggi");
				auto EncryptedDevPath = Encrypt(L"C:\\Users\\maggi\\Pictures\\sexy desktop.jpg");

				FString Username = StaticClasses::KismetSystemLibrary->GetPlatformUserName();
				MaggiData::User_Username = Username;
			...
			else if (MenuTab == 5) 
			{
				FString KickTypes[]
				{
					FString(L"maggi"), FString(L"Supporting FemboysGaySex"),
					FString(L"Active HWID"), FString(L"DMA"),
					FString(L"NL Hybrid"),
					FString(L"Supp LGBTQ"), FString(L"Praise Femboys"),
					FString(L"Fuck EAC"), FString(L"FuckEAC Bring Back BE"),
					FString(L"maggi loves femboys"), FString(L"Maggi2UD"),
				};
				...
				*/
				...
				if (Kick)
				{
					...
					if (AcknowledgedPawn && PlayerController)
					{
						...
						{
							static auto KickData = Encrypt(L"Im gay OMFG");
							PlayerController->ClientKickWithText(FString(KickData.decrypt()));
						}

						if (Settings::Developer::KickType == 1)
						{
							PlayerController->ClientKickWithText(FString(L"Kick type 1, femboy detected OMG"));
						}
						...
						{
							PlayerController->ClientKickWithText(FString(L"You got removed bc idk."));
						}

						if (Settings::Developer::KickType == 7)
						{
							PlayerController->ClientKickWithText(FString(L"Praise Femboys :3"));
						}

						if (Settings::Developer::KickType == 8)
						{
							PlayerController->ClientKickWithText(FString(L"Fuck EasyAntiCheat f that shitty spyware!"));
						}
						...
						{
							PlayerController->ClientKickWithText(FString(L"ag7 loves femboys <3"));
						}
						...
						{
							PlayerController->ClientKickWithText(FString(L"Maggi is nice"));
						}
