...
enum MaggiDebugMessageType : int
{
	Log,
	Info,
	Warn,
...
namespace Custom
{
	...
	*/
	...
	FString GetAimBone(int AimBone)
	{
		...
		else if (Settings::Aimbot::BoneType == 4) 
		{
		...
		else if (Settings::Aimbot::BoneType == 5) 
		{
	...
	double AddOffsetText(double Spacing = 2.0, bool ReturnModifiedVersion = false)
	{
		...
		NewOffset += (Settings::Misc::FontSize + Spacing); 
	...
	FVector2D GetDebugLoc(bool AppendOffset = true, double OffsetToAppend = 2.0)
	{
		...
		return FVector2D(5, 5 + Custom::AddOffsetText(OffsetToAppend)); 
	}

	
	bool DebugUIBegin(FString Watermark = L"maggi :3\r\nUI Debug Mode -> Maggi.\r\n", FLinearColor WatermarkColor = FLinearColor(0.f, 1.0f, 0.f, 1.f))
	{
		
		
		if (Settings::Developer::Debugging::TextOffset != 0) return false; 
	...
	{
		Settings::Developer::Debugging::TextOffset = 0;
		return (Settings::Developer::Debugging::TextOffset == 0); 
	}

	
	FLinearColor GetVColor(MaggiDebugMessageType ColorCodeType_)
	{
		if (ColorCodeType_ == MaggiDebugMessageType::Log) 
			return FLinearColor(1.f, 1.f, 1.f, 1.f); 
		if (ColorCodeType_ == MaggiDebugMessageType::Info) 
			return FLinearColor(1.f, 1.f, 1.f, 1.f); 
		if (ColorCodeType_ == MaggiDebugMessageType::Warn) 
			return FLinearColor(1.f, 0.5f, 0.f, 1.f); 
		if (ColorCodeType_ == MaggiDebugMessageType::Error) 
			return FLinearColor(1.f, 0.0f, 0.f, 1.f); 
		if (ColorCodeType_ == MaggiDebugMessageType::Debug) 
			return FLinearColor(0.f, 1.0f, 0.f, 1.f); 
		if (ColorCodeType_ == MaggiDebugMessageType::Misc) 
			return FLinearColor(0.f, 1.0f, 1.f, 1.f); 
		if (ColorCodeType_ == MaggiDebugMessageType::Other) 
			return FLinearColor(0.f, 1.0f, 1.f, 1.f); 
	...
	void WriteDebugUIStringEx(FString Content, MaggiDebugMessageType ColorCodeType_)
	{
		Custom::WriteDebugUIString(Content, GetVColor(ColorCodeType_)); 
		return;
	}
	...
	{
		WriteDebugUIString_Double(Content, DebugValue, Color); 
		return;
	}
	...
	{
		WriteDebugUIString_Int(Content, DebugValue, Color); 
		return;
	}
	...
	void WriteDebugUIString_StringifiedTimeByFloat(FString Content, float DebugValue, FLinearColor Color = FLinearColor(1.f, 1.f, 1.f, 1.f))
	{
		...
		WriteDebugUIString(ConvertedText, Color); 
		return;
	}
